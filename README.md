...
The **vw** button opens a screen for [uni-stack/uniwind#TBD](https://github.com/uni-stack/uniwind/issues): a `w-[100vw]` bar measured with `onLayout`, a `w-[50vw]` box, and a log line per app-state change showing what uniwind resolved for `100vw` next to `Dimensions.get("window").width`.
